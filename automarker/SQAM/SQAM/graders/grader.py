...
class Grader(ABC):
    """
    Grader is an abstract class that defines the structure a grader must follow.
    """
    ...
    def get_num_extra_columns(self, answer, solutions, max_grade_idx):
        solutions_length = len(solutions[max_grade_idx][0]) if len(
            solutions) > 0 else 0
    ...
    def get_incorrect_answer_details(self, answer, solutions, max_grade_idx):
        ...
        if len(solutions) > 0 and answer:
            missingRows = self.getMissingRows(
                answer[1:], solutions[max_grade_idx][1:])
            extraRows = self.getExtraRows(
                answer[1:], solutions[max_grade_idx][1:])
