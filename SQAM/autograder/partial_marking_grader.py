from SQAM.autograder.grader import Grader
...
class Partial_Marking_Grader(Grader):
    ...
    def __init__(self, assignment, query_language):
        super().__init__(assignment, query_language)
    ...
    def grade_question_for_group(self, group_results, solution_results, max_marks):
        ...
        if group_results == solution_results:
            ...
            group_grade, group_similarity_ratio = self.getMarksForQuestion(group_results, solution_results, max_marks)
        else:
    ...
    def getMarksForQuestion(self, studentAns, solution, maxMarksForQuestion):
        ...
        marks = maxMarksForQuestion if ratio > 0.9 else marks
        return marks, ratio
    ...
    def get_greatest_average_similarity(self, list_of_list_of_all_string_distances):
        ...
        for list_of_string_distances in list_of_list_of_all_string_distances:
            if isinstance(list_of_string_distances, list) and len(list_of_string_distances) > 0:
                score = sum(list_of_string_distances) / len(list_of_string_distances)
    ...
    def getDistance(self, student_results, solution_results):
        ...
        solution_results_lst = sort_list_of_tuples_as_strings(solution_results[1:])
        ...
        for item1, item2 in zip(student_results_lst, solution_results_lst):
            ...
            fuzzy_token_dist.append(self.apply_distance_metric_on_tuples(item1, item2, fuzz.token_sort_ratio))
            jaro_winkler_dist.append(self.apply_distance_metric_on_tuples(item1, item2, textdistance.jaro_winkler))
            ratcliff_dist.append(self.apply_distance_metric_on_tuples(item1, item2, textdistance.ratcliff_obershelp))
        return self.get_greatest_average_similarity([ratcliff_dist, jaro_winkler_dist, fuzzy_token_dist, levenshtein_dist])

    def generate_unit_tests_for_student_groups(self):
        for group in self.assignment.class_list:
            self.generate_test_results_for_group(group)

    def generate_test_results_for_group(self, group):
        test_results = {}
        for q, mark in group.all_grades.items():
            max_mark_for_q = self.assignment.questions[q]
    ...
    def create_similarity_results_as_tests(self, question_number, student_marks, max_marks, ratio, incorrect_answers_details,
        ...
        for i in range(1, student_passes + 1):
            ...
            key_string = self.get_query_correctness_as_string(question_number, percentage_earned)
            value_string = 'Query: {0:} -- Answer is {1:.2f}% correct.'.format(question_number, percentage_earned)
    ...
    def get_failed_test_cases(self, student_passes, max_marks, percent_per_pass, incorrect_answers_details,
        ...
        for i in range(student_passes + 1, max_marks + 1):
            ...
            if i >= max_marks:
                ...
                details_string += '\nColumns Provided:\n\t{}'.format(str(incorrect_answers_details['studentcols']))
